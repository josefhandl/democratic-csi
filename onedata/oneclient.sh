...
do_install() {
	...
	case "$lsb_dist" in
		debian)
			# onedata repo
			$sh_c "apt-get update && apt-get install -y gnupg2"
			$sh_c "apt-key add /tmp/onedata.gpg.key"
			$sh_c "echo \"deb [arch=amd64] ${url}/apt/ubuntu/2002 bionic main\" > /etc/apt/sources.list.d/onedata.list"
			$sh_c "echo \"deb-src [arch=amd64] ${url}/apt/ubuntu/2002 bionic main\" >> /etc/apt/sources.list.d/onedata.list"
		...
		ubuntu-xenial)
			...
			$sh_c "echo \"deb [arch=amd64] ${url}/apt/ubuntu/2002 xenial main\" > /etc/apt/sources.list.d/onedata.list"
			$sh_c "echo \"deb-src [arch=amd64] ${url}/apt/ubuntu/2002 xenial main\" >> /etc/apt/sources.list.d/onedata.list"
		...
		ubuntu-disco)
			# onedata repo
			$sh_c "$curl ${url}/onedata.gpg.key | apt-key add -"
			$sh_c "echo \"deb [arch=amd64] ${url}/apt/ubuntu/2002 disco main\" > /etc/apt/sources.list.d/onedata.list"
			$sh_c "echo \"deb-src [arch=amd64] ${url}/apt/ubuntu/2002 disco main\" >> /etc/apt/sources.list.d/onedata.list"
		...
		fedora)
			# onedata repo
			$sh_c "$curl ${url}/yum/2002/onedata_fedora_29.repo > /etc/yum.repos.d/onedata.repo"
		...
		centos)
			# onedata repo
			$sh_c "$curl ${url}/yum/2002/onedata_centos_7x.repo > /etc/yum.repos.d/onedata.repo"

			$sh_c "yum -y --enablerepo=onedata install ${package}"
			echo_configuration
			exit 0
                        ;;
                "redhatenterpriseserver maipo")
                        # onedata repo
                        $sh_c "$curl ${url}/yum/2002/onedata_centos_7x.repo > /etc/yum.repos.d/onedata.repo"
                        $sh_c "yum -y --enablerepo=onedata install ${package}"
                        echo_configuration
